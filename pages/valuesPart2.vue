<template>
  <div>
    ...
    <section>
      <b-row>
        <b-col>
          <videoPlayer ref="vp" enVideoFile="https://video.csps-efpc.gc.ca/p/101/serveFlavor/entryId/0_4uigcges/flavorId/0_1pjmc3r5/name/a.mp4" frVideoFile="https://video.csps-efpc.gc.ca/p/101/serveFlavor/entryId/0_f8yfrsyq/flavorId/0_m6obd69s/name/a.mp4" chapters chapterFile="ReportPart2.vtt" ccFile="ValuesPart2_captions.vtt" posterFile="m7.jpg" :restartAt="parseInt(thatPoint)" toResume="setReportPart1" :modalArray="modalArray" @timeupdate="updatePercent($event)" />
        </b-col>
      </b-row>
      <b-container>
        <b-row>
    ...
    <div>
      ...
      <b-modal id="directiveConflictInterest" @hide="resumePlay()" size="xl" okOnly>
        <template v-slot:modal-header="{ close }">
          <h3 class="h5">
            <img src="~/assets/InfoIcon.svg" :alt="$t('infoIcon')" width="32" height="32"> {{$t('directiveConflictInterestTitle')}}
          </h3>
          <button type="button" aria-label="Close" class="close" @click="close()">×</button>
        </template>
        <div v-if="$i18n.locale=='en'">
    ...
    <div class="bottomNav reportSection">
      ...
      <microlearning youAreHere :completion="$store.state.currentPlaying.reportPart2_player" path="valuesPart2" imagePath="ValuesPart2.svg" size="100" time="30" :text="$t('ValuesEthics')" type="video" />
